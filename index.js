...
async function createCodeDeployDeployment(
  ...
  let codeDeployAppSpecFile = core.getInput("codedeploy-appspec", {
    required: false,
  });
  codeDeployAppSpecFile = codeDeployAppSpecFile
    ? codeDeployAppSpecFile
    : "appspec.yaml";

  let codeDeployApp = core.getInput("codedeploy-application", {
    required: false,
  });
  codeDeployApp = codeDeployApp
    ? codeDeployApp
    : `AppECS-${clusterName}-${service}`;

  let codeDeployGroup = core.getInput("codedeploy-deployment-group", {
    required: false,
  });
  codeDeployGroup = codeDeployGroup
    ? codeDeployGroup
    : `DgpECS-${clusterName}-${service}`;
  ...
  const appSpecHash = crypto
    .createHash("sha256")
    .update(appSpecString)
    .digest("hex");
  ...
  if (waitForService && waitForService.toLowerCase() === "true") {
    ...
    const deployReadyWaitMin =
      deploymentGroupDetails.blueGreenDeploymentConfiguration
    ...
    core.debug(
      `Waiting for the deployment to complete. Will wait for ${totalWaitMin} minutes`
    );
    await codedeploy
...
async function run() {
  try {
    ...
    const waitForService = core.getInput("wait-for-service-stability", {
      required: false,
    });
    let waitForMinutes =
